...
export default function PopularsPage() {
  ...
  const togglePopular = async (id: string, val: boolean) => {
    await supabase.from('menu_items').update({ is_popular: !val }).eq('id', id)
    await fetch()
  }

  const popular = items.filter(i => i.is_popular)
  const rest = items.filter(i => !i.is_popular)

  return (
    <div>
      <div className="mb-6">
        <h2 className="text-2xl font-bold" style={{ color: '#1A1A1A' }}>Populars</h2>
        <p className="text-sm mt-1" style={{ color: '#888' }}>Featured items shown on homepage — {popular.length} selected</p>
      </div>

      {popular.length > 0 && (
        <div className="mb-6">
          <div className="text-xs font-semibold uppercase tracking-wide mb-3" style={{ color: '#888' }}>⭐ Featured ({popular.length})</div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {popular.map(item => (
              <div key={item.id} className="bg-white rounded-2xl p-4 flex items-center gap-3"
                style={{ border: '1.5px solid #F5C800' }}>
                {item.image_url ? (
                ...
                <button onClick={() => togglePopular(item.id, item.is_popular)}
                  className="p-2 rounded-xl transition-all"
                  style={{ background: '#FFF9E0' }}>
                  <Star size={16} fill="#F5C800" stroke="#F5C800" />
                </button>
              </div>
            ))}
          </div>
      ...
      <div>
        ...
        <div className="bg-white rounded-2xl overflow-hidden" style={{ border: '1px solid #e5e5e5' }}>
          {loading ? (
            ...
            <div className="p-8 text-center text-sm" style={{ color: '#aaa' }}>All items are featured!</div>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: '#fafafa', borderBottom: '1px solid #f0f0f0' }}>
                  {['Item', 'Category', 'Price', 'Feature'].map(h => (
